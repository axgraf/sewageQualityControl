...
class SurrogatevirusQC:
    ...
    def filter_dry_days_time_frame(self, sample_location: str, measurements_df: pd.DataFrame):
        ...
        for sVirus in Columns.get_surrogatevirus_columns():
            measurements_df[Columns.get_surrogate_flag(sVirus)] += np.where((
            ...
            self.logger.log.info("[Surrogatevirus: {}] - [Sample location: '{}'] - \n"
                                 ...
                                 "\tout of {} samples.".
                                 format(sVirus, sample_location, len(measurements_df[measurements_df[Columns.get_surrogate_flag(sVirus)] != 0]),
    ...
    def __get_previous_surrogatevirus_values (self, measurements_df: pd.DataFrame, current_measurement, sVirus):
        ...
        surrogate_flag = current_timeframe[Columns.get_surrogate_flag(sVirus)]
        surrogate_outlier_flag = current_timeframe[Columns.get_surrogate_outlier_flag(sVirus)]
    ...
    def is_surrogatevirus_outlier(self, sample_location: str, measurements_df: pd.DataFrame):
        ...
        progress_bar = self.logger.get_progress_bar(measurements_df.shape[0], "Analyzing surrogatevirus outliers")

        for index, current_measurement in measurements_df.iterrows():
            progress_bar.update(1)
            surrogatevirus_quality_stats['surrogatevirus_total'] += 1
            for sVirus in Columns.get_surrogatevirus_columns():
                if SewageFlag.is_not_flag(current_measurement[Columns.get_surrogate_flag(sVirus)], SewageFlag.SURROGATEVIRUS_VALUE_NOT_USABLE):
                    ...
                    if len(sVirus_values_to_take) > self.min_number_surrogatevirus_for_outlier_detection:
                        ...
                        if is_outlier:
                            measurements_df.at[index, Columns.get_surrogate_outlier_flag(sVirus)] += SewageFlag.SURROGATEVIRUS_OUTLIER.value
                            measurements_df.at[index, Columns.FLAG.value] += SewageFlag.SURROGATEVIRUS_OUTLIER.value
