...
class WaterQuality:
    ...
    def check_water_quality(self, sample_location, measurements_df: pd.DataFrame):
        ...
        plot_water_quality(measurements_df, sample_location, os.path.join(self.output_folder, "water_quality"), self.water_qc_outlier_statistics)
    ...
    def __detect_outliers_in_ammonium(self, sample_location, measurements_df: pd.DataFrame):
        ...
        progress_bar = self.logger.get_progress_bar(measurements_df.shape[0], "Analyzing ammonium outliers")
        for index, current_measurement in measurements_df.iterrows():
            progress_bar.update(1)
            water_quality_stats['total'] += 1
            last_values = get_last_N_month(measurements_df, current_measurement, Columns.AMMONIUM.value,
                                           self.water_quality_number_of_last_month, SewageFlag.AMMONIUM_OUTLIER)
    ...
    def __detect_outliers_in_conductivity(self, sample_location, measurements_df: pd.DataFrame):
        ...
        for k in ['skipped', 'passed', 'failed', 'total']:
            water_quality_stats.setdefault(k, 0)
        progress_bar = self.logger.get_progress_bar(measurements_df.shape[0], "Analyzing conductivity outliers")
        for index, current_measurement in measurements_df.iterrows():
            progress_bar.update(1)
            water_quality_stats['total'] += 1
            last_values = get_last_N_month(measurements_df, current_measurement, Columns.CONDUCTIVITY.value,
                                           self.water_quality_number_of_last_month, SewageFlag.CONDUCTIVITY_OUTLIER)
            enough_last_values = last_values.shape[0] >= self.min_number_of_last_measurements_for_water_qc
            if current_measurement[Columns.CONDUCTIVITY.value] and not math.isnan(current_measurement[Columns.CONDUCTIVITY.value]):  # only if current value is not empty
                if not enough_last_values:
                    water_quality_stats['skipped'] += 1
                    measurements_df.at[index, Columns.FLAG.value] += SewageFlag.NOT_ENOUGH_CONDUCTIVITY_VALUES.value
                else:
                    ...
                    if is_outlier:
                        measurements_df.at[index, Columns.FLAG.value] += SewageFlag.CONDUCTIVITY_OUTLIER.value
                        water_quality_stats['failed'] += 1
                    else:
                        water_quality_stats['passed'] += 1
            else:
